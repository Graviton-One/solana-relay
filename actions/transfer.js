...
async function main() {
  const programId = new anchor.web3.PublicKey(
    "GZh7Rbi4x66yjyW6P5mnLAB1vu4J3vkRJ72fvjnwAiJB"
  );
  const idl = JSON.parse(
    require("fs").readFileSync("../target/idl/relay_port.json", "utf8")
  );
  const program = new anchor.Program(idl, programId);

  const me = anchor.getProvider().wallet.payer;

  const GtonTokenMint = new PublicKey(
    "4hJ6sjwmsvvFag6TKL97yhWiBSDX9BABWoiXgb3EPXxB"
  );
  const tokenProgram = new PublicKey(
    "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA"
  );
  const relayPort = new PublicKey(
    "FjjBrAVteZzeZXnubmKPuCk4FE11Xv9cpc8w7dk5Dp9v"
  );
  ...
  await program.rpc.relay(new BN(200), external_to.toBytes(), destChain, {
    accounts: {
